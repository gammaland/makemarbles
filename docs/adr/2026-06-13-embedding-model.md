...
- **Queries**: short, exploratory, often paraphrased — *"that thing I read about hybrid retrieval"*, not *"hybrid retrieval RRF paper"*.
...
4. **Vector dimensionality** (weight: medium). Lower dims mean smaller index, less RAM, faster cosine — but too low hurts quality. 384–768 is the sweet spot for sqlite-vec at this scale.
...
1. **Build a small eval set** from real dogfood notes — 50–100 marbles, 20 paraphrase-style queries with hand-labeled relevance. Both English and Chinese queries; some mixed-language.
...
- A user-visible failure mode emerges from the e5 prefix discipline — e.g. people running `marbles-mcp` against a fork that forgets the `passage:` prefix and produces silently bad embeddings. If that recurs, we revisit whether the prefix-tuned approach is worth the footgun.
